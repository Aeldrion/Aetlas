execute store result score $upper_bound aetlas run data get storage aetlas:private Function.y.max
...
execute if score $upper_bound aetlas matches 0.. run scoreboard players operation $pos.dy aetlas += $random aetlas

# If no lower bound is specified and the upper bound is negative, the offset is set to the specified upper bound
execute if score $upper_bound aetlas matches ..-1 run scoreboard players operation $pos.dy aetlas += $upper_bound aetlas