execute store result score $lower_bound aetlas run data get storage aetlas:private Function.y.max
...
execute if score $lower_bound aetlas matches ..-1 run scoreboard players operation $pos.dy aetlas += $random aetlas

# If no upper bound is specified and the lower bound is positive, the offset is set to the specified lower bound
execute if score $lower_bound aetlas matches 0.. run scoreboard players operation $pos.dy aetlas += $lower_bound aetlas